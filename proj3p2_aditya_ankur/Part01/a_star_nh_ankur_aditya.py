    ...
    while ol:
        ...
        if eu_dist(node, g_pos) > goal_thr:
            ...
            if end_node is not None:
                ...
                track_info = publish(track)
                track_animate(root,canvas,track,actions)
                robot_animate(root,canvas,track,actions,robot_radius)
                # file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../src/track.txt'))
                with open('./Part02/astar_nh_ankur_aditya/track.txt', 'w') as file:
